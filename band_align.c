...
/*** Returns the best band alignment between two strings
   ...
   * band: size of the band (represents the maximum number of indels
   *       that can be added to s1)
   */
static align_s* band_align(char* s1, char* s2, int64_t band){
	...
	for (; c.prev_x != -1 && c.prev_y != -1; x = c.prev_x, y = c.prev_y, 
			c = m[find_cell(x, y, l1, band)]){
...
int main(int argc, char **argv) {
	...
	/* Band will be initially the minimum number of indel characters to add to s1 */
	int64_t band = l2 - l1;
